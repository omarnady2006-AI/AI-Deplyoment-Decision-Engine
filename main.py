...
def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        dest="dry_run",
        help=(
            "Run the full pipeline analysis but suppress calibration history writes. "
            "Use in CI smoke tests and preview runs to prevent test fixture data from "
            "contaminating the production calibration history."
        ),
    )
...
def run_pipeline(model_path: str, profile_path: str | None = None, dry_run: bool = False) -> PipelineOutput:
    ...
    # I-10: Capture the calibration history snapshot BEFORE any record_calibration()
    # writes so that both recommend_runtime passes (Pass 1 and Pass 2) use an identical,
    # immutable prior.  Records written during this invocation must not contaminate the
    # priors used within the same run.
    _history_snapshot   = load_history(calibration_history_path)
    calibration_stats   = compute_calibration_stats(_history_snapshot)
    runtime_priors      = compute_runtime_priors(_history_snapshot)
    ...
    for runtime in RuntimeName:
        ...
        # I-11: Skip calibration writes when dry_run=True (testing / preview mode) so
        # test fixture models never contaminate the production calibration history.
        if not dry_run:
            record_calibration(
                CalibrationRecord(
    ...
    # I-10: Both recommend_runtime passes use the pre-captured snapshot-derived stats
    # and priors — NOT a fresh load_history() call that would include this run's writes.
    ...
    # I-09: _build_facts() writes 'operator_counts' (plural dict: {op_type: count}).
    # The key 'operator_count' (singular int) was never set, so .get('operator_count', 0)
    # always returned 0.  Derive the scalar by summing the dict values.
    _op_counts_dict    = facts.get("operator_counts", {})
    operator_count_val = (
        sum(_op_counts_dict.values())
        if isinstance(_op_counts_dict, dict) and _op_counts_dict
        else 0
    )
...
def _enforce_decision_gate(result: PipelineOutput, model_path: str) -> tuple[bool, str]:
    ...
    I-08: Every branch that represents a non-deployable decision MUST return
    allowed=False so that main() can exit with code 1 and CI/CD pipelines that
    check standard Unix exit codes correctly block deployment.  Previously all
    branches returned True regardless of decision, making the gate advisory-only
    in name while being a no-op in practice.

    Exit code contract (POSIX-compatible):
        0  APPROVED          — SUPPORTED, deploy
        1  BLOCKED           — UNSUPPORTED / BLOCK, do NOT deploy
        2  CONDITIONAL       — SUPPORTED_WITH_WARNINGS, manual review required
        3  PIPELINE ERROR    — unhandled exception
        4  UNCERTAIN         — insufficient data
    """
    ...
    if decision in (DeploymentDecision.BLOCK,
                    DeploymentDecision.UNSUPPORTED):
        reason = (
            f"BLOCKED: Model '{result.model_name}' is NOT RUNNABLE on any supported runtime.\n"
            f"Decision: {result.decision}\n"
            f"Failures: {result.fail_count}\n"
            f"Warnings: {result.warn_count}\n"
            "Action required: resolve all FAIL-severity diagnostics before deployment."
        )
        return False, reason

    elif decision == DeploymentDecision.SUPPORTED_WITH_WARNINGS:
        reason = (
            f"CONDITIONAL: Model '{result.model_name}' is RUNNABLE WITH CONDITIONS.\n"
            f"Decision: {result.decision}\n"
            f"Failures: {result.fail_count}\n"
            f"Warnings: {result.warn_count}\n"
            "Action required: manual review of all WARN-severity diagnostics."
        )
        return True, reason

    elif decision == DeploymentDecision.SUPPORTED:
        reason = (
            f"APPROVED: Model '{result.model_name}' is RUNNABLE.\n"
            f"Decision: {result.decision}\n"
            f"Failures: {result.fail_count}\n"
            f"Warnings: {result.warn_count}\n"
            "Proceed with standard runtime monitoring."
        )
        return True, reason

    else:
        reason = (
            f"BLOCKED: Unknown decision '{result.decision}' — treating as high risk.\n"
            "Action required: review diagnostics before deployment."
        )
        return False, reason
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        logger.info("cli=pipeline_start model_path=%s", args.model_path)
        # I-11: Pass dry_run so calibration writes are suppressed during test/preview runs.
        result = run_pipeline(args.model_path, profile_path=args.profile_path, dry_run=args.dry_run)
        logger.info(
            "cli=pipeline_complete decision=%s fail_count=%d warn_count=%d status=%s",
            result.decision,
            result.fail_count,
            result.warn_count,
        ...
        # I-08: Gate is now enforced — allowed=False exits with code 1 so every standard
        #       CI/CD platform correctly blocks deployment of non-runnable models.
        allowed, enforcement_reason = _enforce_decision_gate(result, args.model_path)

        print("\n" + "=" * 70)
        print("DEPLOYMENT GATE DECISION")
        print("=" * 70)
        ...
        if not allowed:
            logger.warning(
                "cli=deployment_blocked decision=%s fail_count=%d",
                result.decision, result.fail_count,
            )
            logger.debug("CLI end")
            return 1  # Standard Unix failure — recognised by all CI/CD platforms.

        # Show trust metrics if requested (allowed path only).
        if args.show_trust:
        ...
        # I-08: Standardised exit codes — CONDITIONAL uses 2, UNCERTAIN uses 4.
        ...
        if result.recommendation_status == "UNCERTAIN":
            logger.debug("CLI end")
            return 4
        # CONDITIONAL_APPROVAL or any other non-error non-block outcome.
        logger.debug("CLI end")
        return 2

    except Exception as error:
        logger.exception("cli=pipeline_error reason=%s", str(error))
