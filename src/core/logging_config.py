...
# Keys that exist on every logging.LogRecord.  Passing any of these via
# extra={} raises KeyError: "Attempt to overwrite '<key>' in LogRecord".
# Keep in sync with the _JsonFormatter._RESERVED set below.
...
def safe_extra(extra: dict[str, Any]) -> dict[str, Any]:
    """
    Return a copy of *extra* safe to pass to any logger.* call.

    Any key that collides with a LogRecord reserved field is prefixed
    with ``ctx_`` so the value is preserved and the crash is avoided.

    Example::

        logger.info("upload", extra=safe_extra({"filename": f.filename}))
        # → logged as {"ctx_filename": "model.onnx", ...}

    Use this at every call-site that constructs extra= from user-supplied
    or externally-sourced data where key names cannot be guaranteed safe.
    """
    return {
        (f"ctx_{k}" if k in RESERVED_LOG_KEYS else k): v
