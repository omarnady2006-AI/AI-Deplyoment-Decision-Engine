...
def _generate_approved_recommendation(input_data: AdvisorInput) -> str:
    return (
        ...
        f"1. Deploy the model to your target environment\n"
        f"2. Monitor initial inference performance\n"
        f"3. Validate output accuracy with test data"
    )
...
def _generate_conditional_recommendation(input_data: AdvisorInput) -> str:
    ...
    return (
        f"This model has conditional approval for deployment.\n\n"
        f"Conditions to address:\n{reasons}\n\n"
        f"Recommended actions:\n"
        f"1. Review the conditions above\n"
        f"2. Test thoroughly in a staging environment\n"
        f"3. Consider optimizations to address warnings"
    )
...
def _generate_rejection_analysis(input_data: AdvisorInput) -> str:
    ...
    return (
        f"This model was rejected for deployment.\n\n"
        f"Rejection reasons:\n{reasons}\n\n"
        f"Recommended actions:\n"
        f"1. Address the issues listed above\n"
        f"2. Consider model optimization or alternative runtimes\n"
        f"3. Re-run analysis after changes"
    )
...
def _generate_unsupported_ops_section(input_data: AdvisorInput) -> str:
    ...
    return (
        f"The following operators may not be supported on all backends:\n\n"
        f"{ops}\n\n"
        f"Consider verifying operator support for your target runtime."
    )
