...
# ── Resource-limit constants (Patch Groups 2 & 8) ─────────────────────────
_MIN_FREE_DISK_BYTES = 512 * 1024 * 1024        # 512 MB minimum free disk
_MAX_MODEL_LOAD_BYTES = 2 * 1024 * 1024 * 1024  # 2 GB max file size
_MAX_THREAD_POOL_WORKERS = 64  # PG8.2 – global thread pool ceiling
_MEMORY_MULTIPLIER = {"pytorch": 3.5, "onnx": 2.0, "tensorflow": 2.5}

# ── Circuit-breaker state (Patch 2.5) ──────────────────────────────────────
_CIRCUIT_BREAKER_LOCK: threading.Lock = threading.Lock()
_CIRCUIT_FAILURE_COUNT: int = 0
_CIRCUIT_LAST_RESET: float = 0.0
_CIRCUIT_OPEN: bool = False
_CIRCUIT_FAILURE_THRESHOLD: int = 5
_CIRCUIT_RESET_SECONDS: float = 60.0

def _circuit_breaker_record_failure() -> None:
    global _CIRCUIT_FAILURE_COUNT, _CIRCUIT_OPEN, _CIRCUIT_LAST_RESET
    with _CIRCUIT_BREAKER_LOCK:
        now = time.monotonic()
        if now - _CIRCUIT_LAST_RESET > _CIRCUIT_RESET_SECONDS:
            _CIRCUIT_FAILURE_COUNT = 0
            _CIRCUIT_LAST_RESET = now
            _CIRCUIT_OPEN = False
...
def _circuit_breaker_check() -> bool:
    ...
    global _CIRCUIT_OPEN, _CIRCUIT_FAILURE_COUNT, _CIRCUIT_LAST_RESET
    with _CIRCUIT_BREAKER_LOCK:
        now = time.monotonic()
        if _CIRCUIT_OPEN and (now - _CIRCUIT_LAST_RESET > _CIRCUIT_RESET_SECONDS):
            _CIRCUIT_OPEN = False
            _CIRCUIT_FAILURE_COUNT = 0
            _CIRCUIT_LAST_RESET = now
        return _CIRCUIT_OPEN

def _check_disk_space(path: str = "/tmp") -> tuple[bool, str]:
...
def profile_load_saturation(model_path: str, baseline_latency_ms: float = 0.0) -> dict[str, Any]:
    ...
    try:
        ...
        for level in levels:
            ...
            with ThreadPoolExecutor(max_workers=min(level, _MAX_THREAD_POOL_WORKERS)) as ex:
                futures = []
...
def analyze_model_security(model_path: str) -> dict[str, Any]:
    ...
    try:
        # Block external data references (Patch 8.1)
        file_size = os.path.getsize(model_path)
        if file_size > _MAX_MODEL_LOAD_BYTES:
            s["security_penalty"] = 10.0
            return s

        model = onnx.load(model_path)

        # Reject models referencing external data (Patch 8.1)
        for init in getattr(getattr(model, "graph", None), "initializer", []) or []:
            if getattr(init, "data_location", 0) == 1:  # EXTERNAL
                s["security_penalty"] = 10.0
                logger.info("stage=security status=blocked reason=external_data_reference")
                return s
        graph = model.graph
        unsafe_set = {
            "PythonOp",
...
def measure_concurrency_pressure(model_path: str, baseline_latency_ms: float, workers: int = 4) -> dict[str, Any]:
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=min(workers, _MAX_THREAD_POOL_WORKERS)) as ex:
...
def _profile_concurrency_probe(
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=min(workers, _MAX_THREAD_POOL_WORKERS)) as pool:
...
def profile_model_runtime(
    ...
    # Compute budget_ms early so early-exit guards can use it
    _early_budget_ms = max(50.0, float(profiling_budget_ms or 2000.0))
    _early_default: dict[str, Any] = {"profiling_time_ms": 0.0}

    # ── Patch 2.5: Circuit-breaker check ─────────────────────────────────────
    if _circuit_breaker_check():
        logger.warning("stage=profiling status=rejected reason=circuit_open")
        return _finalize_profiling_result(_early_default, 0.0, _early_budget_ms, request_id=request_id)

    # ── Patch 2.2: FD exhaustion guard ───────────────────────────────────────
    fd_ok, fd_reason = _check_fd_pressure()
    if not fd_ok:
        logger.warning("stage=profiling status=rejected reason=%s", fd_reason)
        return _finalize_profiling_result(_early_default, 0.0, _early_budget_ms, request_id=request_id)

    # ── Patch 2.3: Disk space guard ──────────────────────────────────────────
    disk_ok, disk_reason = _check_disk_space()
    if not disk_ok:
        logger.warning("stage=profiling status=rejected reason=%s", disk_reason)
        return _finalize_profiling_result(_early_default, 0.0, _early_budget_ms, request_id=request_id)

    # ── Patch 1.5: File size guard BEFORE any load ───────────────────────────
    try:
        _model_file_size = os.path.getsize(model_path)
        if _model_file_size > _MAX_MODEL_LOAD_BYTES:
            logger.info("stage=profiling status=failed reason=file_too_large size=%d", _model_file_size)
            return _finalize_profiling_result({"profiling_time_ms": 0.0}, 0.0, _early_budget_ms, request_id=request_id)
    except OSError as e:
        logger.warning("exception_occurred", exc_info=True)
    required_signals = (
        "latency",
    ...
    logger.info("stage=profile status=start path=%s budget_ms=%.0f request_id=%s",
                model_path, budget_ms, request_id)
    ...
    try:
        ...
        if _fw != "onnx":
            ...
            if not _model_was_cached:
                # PG4.1 – parse time guard: measure elapsed for belt-and-suspenders
                _parse_t0 = time.perf_counter()
                logger.info("stage=parse status=start path=%s", model_path)
                try:
                    _loaded_model = onnx.load(model_path)
                except MemoryError:
                    # PG4.2 – memory error → downgrade, never retry
                    logger.warning("stage=parse status=failed reason=memory_error; downgrading to static_only")
                    default_result["execution_mode"] = "static_only"
                    return _finalize_profiling_result(default_result, 0.0, budget_ms, request_id=request_id)
                _parse_elapsed = time.perf_counter() - _parse_t0
                if _parse_elapsed > _MAX_MODEL_LOAD_BYTES / (100 * 1024 * 1024):
                    # Heuristic: >1s/100MB suggests I/O pressure (PG4.3)
                    logger.warning("stage=parse status=slow elapsed=%.2fs", _parse_elapsed)
                logger.info("stage=parse status=complete elapsed_ms=%.0f", _parse_elapsed * 1000)
                with _ONNX_MODEL_CACHE_LOCK:
        ...
        logger.info("stage=profile status=complete path=%s elapsed_ms=%.0f signals=%d request_id=%s",
                    model_path, _profiling_elapsed_s() * 1000.0,
                    result.get("measured_signal_count", 0), request_id)
        ...
        logger.error("stage=profiling status=exception reason=%s", str(e))
...
def simulate_input_abuse(model_path: str) -> dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            logger.warning("ort_invalid_type_rejected_as_expected", exc_info=True)  # Expected: ORT rejects bad types gracefully
        ...
        try:
            n_threads = min(20, MAX_PROFILE_THREADS, _MAX_THREAD_POOL_WORKERS)
