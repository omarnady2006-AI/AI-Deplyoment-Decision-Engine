...
def run_pipeline(
    ...
    try:
        ...
        resolved_hash = model_hash if model_hash is not None else compute_model_hash(model_path)
        logger.info(f"pipeline_start", extra={"model_hash": resolved_hash, "model_path": model_path})
...
def _run_verification(n_scenarios: int = 50000, seed: int = 42) -> None:
    ...
    print(f"  C value : 0.0890  (Phase B fix: smooth_norm_ratio=ratio/(1+ratio), verified 50k)")
    print()
